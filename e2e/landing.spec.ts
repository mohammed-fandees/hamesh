...
test.describe('Landing page', () => {
  ...
  test('never shows the page before its animations are ready', async ({ page }) => {
    ...
    // Sampled from the first commit: the heading must never be painted in its
    // finished position and then yanked back to the start of its entrance.
    ...
    expect(samples.some((s) => s.shown)).toBe(true);
